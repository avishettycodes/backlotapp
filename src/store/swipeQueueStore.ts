...
interface SwipeQueueStore {
  ...
  removeFromQueue: (carId: number | string) => void
...
export const useSwipeQueueStore = create<SwipeQueueStore>((set) => ({
  carQueue: [],
  addToQueue: (car) => {
    if (__DEV__) console.log('Store: Adding car to swipe queue:', car)
    set((state) => {
      // Check if car is already in queue
      const isAlreadyInQueue = state.carQueue.some((existingCar) => existingCar.id === car.id)
      if (isAlreadyInQueue) {
        if (__DEV__) console.log('Car already in queue, skipping...')
        return state
      }
      
      const newState = { carQueue: [...state.carQueue, car] }
      if (__DEV__) console.log('Store: New queue state:', newState)
      return newState
    })
  },
  removeFromQueue: (carId) => {
    if (__DEV__) console.log('Store: Removing car from queue:', carId)
    set((state) => {
      const newState = {
        carQueue: state.carQueue.filter((car) => car.id !== Number(carId)),
      }
      if (__DEV__) console.log('Store: New queue state after removal:', newState)
      return newState
    })
  },
  initializeQueue: (cars) => {
    if (__DEV__) console.log('Store: Initializing queue with cars:', cars)
    set({ carQueue: cars })
  },
  clearQueue: () => {
    if (__DEV__) console.log('Clearing swipe queue...')
    set({ carQueue: [] })
  },
})) 